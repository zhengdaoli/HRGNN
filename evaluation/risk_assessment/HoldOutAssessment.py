...
class HoldOutAssessment:
    ...
    def risk_assessment(self, experiment_class, debug=False, other=None, pretrain=False):
        """
        ...
        self._risk_assessment_helper(experiment_class, self._HOLDOUT_FOLDER, debug, other, pretrain=pretrain)

    def _risk_assessment_helper(self, experiment_class, exp_path, debug=False, other=None, pretrain=False):

        dataset_getter = DatasetGetter(None)

        if pretrain:
            best_config = self.model_selector.load_best_config()
        else:
        ...
        for i in range(3):
            training_score, test_score = experiment.run_test(dataset_getter, logger, other, pretrain=pretrain)
        ...
        if pretrain:
            with open(os.path.join(self._HOLDOUT_FOLDER, "pretrain_"+self._ASSESSMENT_FILENAME), 'w') as fp:
