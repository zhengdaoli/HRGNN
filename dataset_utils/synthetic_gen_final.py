...
def dump_ER_graphs_by_CC(sample_num, class_num, rs, er_nodes=100, min_CC=0.1, max_CC=0.5, name=None):
    ...
    for i in range(sample_num):
        tr_num = int(tris[i])
        G = nx.erdos_renyi_graph(er_nodes, 0.02)
        ...
        # 3 tri, 4 sq, 9+16, 4 tri 3 sq, 12+12, 24,
        # x*3 + 4*y = 100, nod 
    ...
    # # Y cut half:
    # cc_sample = zip(samples, cc_cur)
    # cc_sample = sorted(cc_sample, key=lambda x: x[1])
    # Y = np.ones((sample_num, 1))
    # Y[:len(cc_sample)//2] = 0
    # samples = [s[0] for s in cc_sample]    
...
if __name__ == '__main__':
    ...
    for i in range(1, 10):
        correlation = i/10
        print(correlation)
        ok = dump_ER_graphs_by_CC(2048, class_num, rs=[correlation], name=f'final_syn_{round(correlation, 1)}_class{class_num}')
